...
class MetricsTester:
    def __init__(self, dure_time, target):
        self.target = target
        self.time = time
        ...
        self.cpu_line, = self.ax1.plot([], [], lw=2)
        self.cpu_data = []
        self.timestamp=[]
        plt.ion()
    ...
    def animate(self, timestamp):
        ...
        self.cpu_data.append(12)
        print(self.cpu_data)
        self.cpu_line.set_data(self.timestamp, self.cpu_data)
        return self.cpu_line,

    def start(self):
        # self.get_title_info()
        ani = animation.FuncAnimation(self.fig, self.animate, interval=100, init_func=self.init_data)

        plt.show()

    def cal_average(self, lists):
        sums = 0
        for i in lists:
            sums += i
...
if __name__ == '__main__':
    mt = MetricsTester(5, "com.netease.stz+")  # 输入采样持续时间，单位秒tv.danmaku.bili
    mt.start()
